package br.com.fiap.carstation;
...
import br.com.fiap.carstation.controller.ButtonController;
import br.com.fiap.carstation.controller.TableController;
...
public class App extends JFrame {
	...
	public void loadData() {
		...
		List<Estacao> list = new EstacaoDao().showAll();
		list.forEach(estacao -> tableModel.addRow(estacao.getData()));

		postosSelector.removeAllItems();
		postosSelector.addItem("Selecione um");
		list.forEach(estacao -> postosSelector.addItem(estacao.getData().get(2)));
	}

	public void loadDataOrdered() {
		tableModel.setRowCount(0);
		List<Estacao> list = new EstacaoDao().orderByStates();
		list.forEach(estacao -> tableModel.addRow(estacao.getData()));
	}
}
